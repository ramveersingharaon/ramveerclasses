...
//   // Firebase auth check - just like in Navbar
//   useEffect(() => {
//     const auth = getAuth(app);
//     const unsubscribe = onAuthStateChanged(auth, (user) => {
//       setIsAdmin(!!user); // agar user logged in hai to admin true
//     });
...
//         // Filter only 12th class notes
//         const filtered = data.filter((note: Note) => note.className === "12");
//         setNotes(filtered);
//       } catch (error) {
...
//         </div>
//       ) : notes.length === 0 ? (
//         <p className="text-center text-gray-500">No 12th class notes found.</p>
//       ) : (
...
export default function TwelfthNotesPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      `}</style>
      <h1 className="text-3xl font-bold text-center text-blue-800 mb-8">12th Class Notes</h1>
      {fetching && displayedNotes.length === 0 ? (
        <div className="flex justify-center items-center h-40">
          <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin" />
        </div>
      ) : allNotes.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No 12th class notes found.</p>
      ) : (

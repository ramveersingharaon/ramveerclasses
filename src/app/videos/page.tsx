...
        const data = await res.json();
        if (data?.videos && Array.isArray(data.videos)) {
          const classes = Array.from(new Set(data.videos.map((video: Video) => video.className).filter(Boolean))) as string[];
          setAvailableClasses(classes.sort((a, b) => parseInt(b) - parseInt(a)));

          const subjects = Array.from(new Set(data.videos.map((video: Video) => video.subjectName).filter(Boolean))) as string[];

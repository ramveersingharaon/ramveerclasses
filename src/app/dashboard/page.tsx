...
// The classes and subjects array are defined here, can be moved to a separate file if needed.
      ...
      {showNoteModal && (
        <Modal title="Add New Note" onClose={() => setShowNoteModal(false)}>
          <form onSubmit={uploadNote} className="space-y-4">
            ...
            <Textarea label="Description" value={noteDescription} onChange={setNoteDescription} />
...
function FileInput({ label, accept, onChange }: { label: string; accept: string; onChange: (file: File | null) => void; }) {
  return (
    <div>
      <label className="block mb-1 font-semibold text-gray-900">{label}</label>
      <input
